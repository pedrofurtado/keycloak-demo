...
export function Admin() {
  ...
  const headers_refresh_jwt = JSON.stringify(decodeProtectedHeader(Cookies.get("refresh_token")), null, 2);
  const payload_refresh = JSON.stringify(decodeJwt(Cookies.get("refresh_token")), null, 2);
  ...
  return (
    <div>
      ...
      <pre>{payload_id}</pre>
      <br/>==================================<br/>
      <br/>JWT refresh token - Headers<br/>
      <pre>{headers_refresh_jwt}</pre>
      <br/>JWT refresh token - Payload<br/>
      <pre>{payload_refresh}</pre>
    </div>
  );
}
